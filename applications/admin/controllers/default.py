...
# from gluon.tools import Config # FIX THIS
# from gluon.compileapp import find_exposed_functions # FIX THIS
...
def change_password():
    ...
    if session.pam_user:
        ...
        redirect(URL('site'))
    form = Form([Field('current_admin_password', 'password'),
                 Field('new_admin_password',
                       'password', requires=IS_STRONG()),
                 Field('new_admin_password_again', 'password')])
    #form['_class']="span4 well" # FIX THIS
    if form.accepted:
...
def site():
    ...
    file_or_appurl = 'file' in request.post_vars or 'appurl' in request.post_vars
    ...
    form_create = Form([Field('name', requires=is_appname)],formname='appcreate',csrf=False)
    form_update = Form([Field('name', requires=is_appname),
                        Field('file', 'upload', uploadfield=False),
                        Field('url'),
                        Field('overwrite', 'boolean')],
                       formname='appupdate',csrf=False)
...
def edit():
    ...
    if len(request.args) > 2 and request.args[1] == 'controllers':
        ...
        functions = [] # find_exposed_functions(data) # FIX THIS
...
def design():
    ...
    for c in controllers:
        ...
        items = [] #find_exposed_functions(data) # FIX THIS
    ...
    for c in views:
        ...
        items = [] # //regex_extend.findall(data) # FIX THIS

        if items:
            extend[c] = items[0][1]

        items = [] # regex_include.findall(data) # FIX THIS
...
def manage_students():
    ...
    grid = SQLFORM.grid(db.auth_user) # FIX THIS
    return locals()
