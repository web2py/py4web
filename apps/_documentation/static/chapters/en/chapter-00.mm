...
- PY4WEB, unlike web2py, can be installed using pip and its dependecies are managed using requirements.txt
- PY4WEB apps are regular python modules. This is very different than web2py. In particular, we ditched the custom importer, and we rely now exclusively on the regular Python import mechanism.
...
- PY4WEB, unlike web2py, does not create a new enviroment at every request. It introduces the concept of fixtures to explicitly declare which objects need to be re-initialized when a new http request is processed. This makes it much faster.
...
- PY4WEB comes with a Dashboard APP that replaces web2py's admin. This is a web IDE for uploading/managing/editings apps.
