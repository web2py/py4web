...
- we re-assembled some of those modules into a new more modular framework ... PY4WEB.
...
- PY4WEB, unlike web2py, can be installed using pip and its dependecies are manages using requirements.txt
- PY4WEB apps are regular python modules. This is very different than web2py. In particular we ditched the custom importer and rely exclusively on the regular Python import mechanism.
...
- PY4WEB, unlike web2py, does not create a new enviroment at every request. It introduces the concept of fixtures to explicitly declare which objects needs to be re-initialized when a new http request is processed. This makes it much faster.
- PY4WEB, has a new sesson object which, like web2py's, provide strong security and encryption of the session data, but sessions are no longer stored on file system which created performance issues. It provides sessions in cookies, in redis, in memcache, or in database. We also limited session data to objects that are json serializable.
- PY4WEB, like web2py, has a built-in ticketing system but, unlike web2py, this system is global and not per app. Tickets are no longer stored on filesystem with the individual apps. They are stored in a single database.
- PY4WEB, like web2py, is based on pydal but uses some new features of pydal (RESTAPI)
- PY4WEB, like web2py, uses the yatl template language but defaults to square brackets [[...]] delimiters to avoid conflicts with model JS fameworks such as Vue.js and angular.js. Yatl includes a subset of the web2py helpers.
...
- PY4WEB comes with Dashboard APP that replaces web2py's admin. This is a web IDE for uploading/managing/editings apps.
...
- PY4WEB comes with some utilites like "tags" which allows adding searchable tags to any database table. It can be used, for example, to tag users with groups and search users by groups and apply permissions based on membership.
