...
- WEB3PY apps are regular python modules. This is very different than web2py. In particular we ditched the custom importer and rely exclusively on the regular Python import mechanism.
...
- WEB3PY, unlike web2py, does not create a new enviroment at every request. It introduces the concept of fixtures to explicitly declare which objects needs to be re-initialized when a new http request is processed. This makes it much faster.
- WEB3PY, has a new sesson object which, like web2py's, provide strong security and encryption of the session data, but sessions are no longer stored on file system which created performance issues. It provides sessions in cookies, in redis, in memcache, or in database. We also limited session data to objects that are json serializable.
- WEB3PY, like web2py, has a built-in ticketing system but, unlike web2py, this system is global and not per app. Tickets are no longer stored on filesystem with the individual apps. They are stored in a single database.
