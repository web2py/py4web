...
- the Session object (which tells py4web to parse the cookie and retrieve a session  at every request, and save it if changed)
...
In your html you can use any JS library that you choose because py4web is agnostic to your choice of JS and CSS but with some exceptions. The ``auth.html`` which handles registration/login/etc. uses a vue.js component. Hence if you want to use that you should not remove it.
