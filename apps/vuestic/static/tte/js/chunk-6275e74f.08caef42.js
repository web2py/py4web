(window["webpackJsonp"] = window["webpackJsonp"] || []).push([
    ["chunk-6275e74f"], {
        8693: function(t, l, i) {
            "use strict";
            var e = function() {
                    var t = this,
                        l = t.$createElement,
                        i = t._self._c || l;
                    return i("div", {
                        staticClass: "line-map fill-height"
                    }, [t._v(" Line Map ")])
                },
                o = [],
                a = (i("2439"), i("28db"), i("7d4b"), i("0d41"), i("0c0a"), i("5dba"), {
                    name: "line-map",
                    props: ["mapData"],
                    watch: {
                        mapData: function() {
                            this.addDataToMap(), this.map.validateData()
                        }
                    },
                    data: function() {
                        return {
                            dataProvider: {
                                mapVar: AmCharts.maps.worldLow
                            }
                        }
                    },
                    computed: {
                        map: function() {
                            return new AmCharts.AmMap
                        }
                    },
                    methods: {
                        drawMap: function() {
                            this.map.areasSettings = {
                                unlistedAreasColor: "#eee",
                                unlistedAreasAlpha: 1,
                                outlineColor: "#fff",
                                outlineThickness: 2
                            }, this.map.imagesSettings = {
                                color: this.$themes.info,
                                rollOverColor: this.$themes.info,
                                selectedColor: this.$themes.primary
                            }, this.map.linesSettings = {
                                color: this.$themes.info,
                                alpha: .4
                            }, this.addDataToMap(), this.map.dataProvider = this.dataProvider, this.map.backgroundZoomsToTop = !0, this.map.linesAboveImages = !0, this.map.write(this.$el)
                        },
                        addDataToMap: function() {
                            this.dataProvider.linkToObject = this.mapData.mainCity, this.dataProvider.images = this.mapData.cities
                        }
                    },
                    mounted: function() {
                        this.drawMap()
                    }
                }),
                n = a,
                s = i("2877"),
                u = Object(s["a"])(n, e, o, !1, null, null, null);
            l["a"] = u.exports
        },
        c826: function(t, l, i) {
            "use strict";
            i.r(l);
            var e = function() {
                    var t = this,
                        l = t.$createElement,
                        i = t._self._c || l;
                    return i("div", {
                        staticClass: "line-maps-page"
                    }, [i("div", {
                        staticClass: "row"
                    }, [i("div", {
                        staticClass: "flex md12 xs12"
                    }, [i("va-card", {
                        staticClass: "line-maps-page__widget",
                        attrs: {
                            title: "Line Maps"
                        }
                    }, [i("line-map", {
                        staticStyle: {
                            height: "65vh"
                        },
                        attrs: {
                            "map-data": t.lineMapData
                        }
                    })], 1)], 1)])])
                },
                o = [],
                a = i("8693"),
                n = i("ff60"),
                s = {
                    name: "line-maps-page",
                    components: {
                        LineMap: a["a"]
                    },
                    computed: {
                        lineMapData: function() {
                            return Object(n["a"])(this.$themes)
                        }
                    }
                },
                u = s,
                d = i("2877"),
                r = Object(d["a"])(u, e, o, !1, null, null, null);
            l["default"] = r.exports
        },
        ff60: function(t, l, i) {
            "use strict";
            i.d(l, "a", (function() {
                return e
            }));
            var e = function(t) {
                var l = "M9,0C4.029,0,0,4.029,0,9s4.029,9,9,9s9-4.029,9-9S13.971,0,9,0z M9,15.93 c-3.83,0-6.93-3.1-6.93-6.93S5.17,2.07,9,2.07s6.93,3.1,6.93,6.93S12.83,15.93,9,15.93 M12.5,9c0,1.933-1.567,3.5-3.5,3.5S5.5,10.933,5.5,9S7.067,5.5,9,5.5 S12.5,7.067,12.5,9z",
                    i = "M19.671,8.11l-2.777,2.777l-3.837-0.861c0.362-0.505,0.916-1.683,0.464-2.135c-0.518-0.517-1.979,0.278-2.305,0.604l-0.913,0.913L7.614,8.804l-2.021,2.021l2.232,1.061l-0.082,0.082l1.701,1.701l0.688-0.687l3.164,1.504L9.571,18.21H6.413l-1.137,1.138l3.6,0.948l1.83,1.83l0.947,3.598l1.137-1.137V21.43l3.725-3.725l1.504,3.164l-0.687,0.687l1.702,1.701l0.081-0.081l1.062,2.231l2.02-2.02l-0.604-2.689l0.912-0.912c0.326-0.326,1.121-1.789,0.604-2.306c-0.452-0.452-1.63,0.101-2.135,0.464l-0.861-3.838l2.777-2.777c0.947-0.947,3.599-4.862,2.62-5.839C24.533,4.512,20.618,7.163,19.671,8.11z",
                    e = {
                        id: "london",
                        color: t.primary,
                        svgPath: l,
                        title: "London",
                        country: "United Kingdom",
                        latitude: 51.5002,
                        longitude: -.1262,
                        scale: 1.5,
                        zoomLevel: 2.74,
                        zoomLongitude: -20.1341,
                        zoomLatitude: 49.1712,
                        lines: [{
                            latitudes: [51.5002, 50.4422],
                            longitudes: [-.1262, 30.5367]
                        }, {
                            latitudes: [51.5002, 46.948],
                            longitudes: [-.1262, 7.4481]
                        }, {
                            latitudes: [51.5002, 59.3328],
                            longitudes: [-.1262, 18.0645]
                        }, {
                            latitudes: [51.5002, 40.4167],
                            longitudes: [-.1262, -3.7033]
                        }, {
                            latitudes: [51.5002, 46.0514],
                            longitudes: [-.1262, 14.506]
                        }, {
                            latitudes: [51.5002, 48.2116],
                            longitudes: [-.1262, 17.1547]
                        }, {
                            latitudes: [51.5002, 44.8048],
                            longitudes: [-.1262, 20.4781]
                        }, {
                            latitudes: [51.5002, 55.7558],
                            longitudes: [-.1262, 37.6176]
                        }, {
                            latitudes: [51.5002, 38.7072],
                            longitudes: [-.1262, -9.1355]
                        }, {
                            latitudes: [51.5002, 54.6896],
                            longitudes: [-.1262, 25.2799]
                        }, {
                            latitudes: [51.5002, 64.1353],
                            longitudes: [-.1262, -21.8952]
                        }, {
                            latitudes: [51.5002, 40.43],
                            longitudes: [-.1262, -74]
                        }],
                        images: [{
                            label: "Flights from London",
                            svgPath: i,
                            left: 100,
                            top: 45,
                            color: t.info,
                            labelColor: t.info,
                            labelRollOverColor: t.info,
                            labelFontSize: 20
                        }, {
                            label: "show flights from Vilnius",
                            left: 110,
                            top: 70,
                            labelColor: t.primary,
                            labelRollOverColor: t.info,
                            labelFontSize: 11,
                            linkToObject: "vilnius"
                        }]
                    },
                    o = {
                        id: "vilnius",
                        color: t.primary,
                        svgPath: l,
                        title: "Vilnius",
                        country: "Lithuania",
                        latitude: 54.6896,
                        longitude: 25.2799,
                        scale: 1.5,
                        zoomLevel: 4.92,
                        zoomLongitude: 15.4492,
                        zoomLatitude: 50.2631,
                        lines: [{
                            latitudes: [54.6896, 50.8371],
                            longitudes: [25.2799, 4.3676]
                        }, {
                            latitudes: [54.6896, 59.9138],
                            longitudes: [25.2799, 10.7387]
                        }, {
                            latitudes: [54.6896, 40.4167],
                            longitudes: [25.2799, -3.7033]
                        }, {
                            latitudes: [54.6896, 50.0878],
                            longitudes: [25.2799, 14.4205]
                        }, {
                            latitudes: [54.6896, 48.2116],
                            longitudes: [25.2799, 17.1547]
                        }, {
                            latitudes: [54.6896, 44.8048],
                            longitudes: [25.2799, 20.4781]
                        }, {
                            latitudes: [54.6896, 55.7558],
                            longitudes: [25.2799, 37.6176]
                        }, {
                            latitudes: [54.6896, 37.9792],
                            longitudes: [25.2799, 23.7166]
                        }, {
                            latitudes: [54.6896, 54.6896],
                            longitudes: [25.2799, 25.2799]
                        }, {
                            latitudes: [54.6896, 51.5002],
                            longitudes: [25.2799, -.1262]
                        }, {
                            latitudes: [54.6896, 53.3441],
                            longitudes: [25.2799, -6.2675]
                        }],
                        images: [{
                            label: "Flights from Vilnius",
                            svgPath: i,
                            left: 100,
                            top: 45,
                            color: t.info,
                            labelColor: t.info,
                            labelRollOverColor: t.info,
                            labelFontSize: 20
                        }, {
                            label: "show flights from London",
                            left: 110,
                            top: 70,
                            labelColor: t.primary,
                            labelRollOverColor: t.info,
                            labelFontSize: 11,
                            linkToObject: "london"
                        }]
                    },
                    a = [e, o, {
                        svgPath: l,
                        color: t.info,
                        title: "Brussels",
                        country: "Belgium",
                        latitude: 50.8371,
                        longitude: 4.3676
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Prague",
                        country: "Czech Republic",
                        latitude: 50.0878,
                        longitude: 14.4205
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Athens",
                        country: "Greece",
                        latitude: 37.9792,
                        longitude: 23.7166
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Reykjavik",
                        country: "Iceland",
                        latitude: 64.1353,
                        longitude: -21.8952
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Dublin",
                        country: "Ireland",
                        latitude: 53.3441,
                        longitude: -6.2675
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Oslo",
                        country: "Norway",
                        latitude: 59.9138,
                        longitude: 10.7387
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Lisbon",
                        country: "Portugal",
                        latitude: 38.7072,
                        longitude: -9.1355
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Moscow",
                        country: "Russia",
                        latitude: 55.7558,
                        longitude: 37.6176
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Belgrade",
                        country: "‎Serbia",
                        latitude: 44.8048,
                        longitude: 20.4781
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Bratislava",
                        country: "Slovakia",
                        latitude: 48.2116,
                        longitude: 17.1547
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Ljubljana",
                        country: "Slovenia",
                        latitude: 46.0514,
                        longitude: 14.506
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Madrid",
                        country: "Spain",
                        latitude: 40.4167,
                        longitude: -3.7033
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Stockholm",
                        country: "Sweden",
                        latitude: 59.3328,
                        longitude: 18.0645
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Bern",
                        country: "Switzerland",
                        latitude: 46.948,
                        longitude: 7.4481
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Kiev",
                        country: "Ukraine",
                        latitude: 50.4422,
                        longitude: 30.5367
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "Paris",
                        country: "France",
                        latitude: 48.8567,
                        longitude: 2.351
                    }, {
                        svgPath: l,
                        color: t.info,
                        title: "New York",
                        country: "United States of America",
                        latitude: 40.43,
                        longitude: -74
                    }];
                return {
                    cities: a,
                    mainCity: e
                }
            }
        }
    }
]);
