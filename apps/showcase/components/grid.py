...
class Grid(Fixture):
    ...
    def api(self):
        ...
        search_text = request.query.get("q", "").lower()  # Query string
        ...
        rows = [
            dict(
                cells=[
                    dict(text="Cat"),
                    dict(text="4"),
                    dict(text="Mammal", url="javascript:alert('mammals/cat')", is_button=True),
                ]
            ),
            dict(
                cells=[
                    dict(text="Dog"),
                    dict(text="4"),
                    dict(text="Mammal", url="javascript:alert('mammals/dog')", is_button=True),
                ]
            ),
            dict(
                cells=[
        ...
        if search_text:
            rows = [row for row in rows if search_text in row['cells'][0]['text'].lower()]
        if sort_order:
            for i,j in enumerate(sort_order):
                if j != 0:
                    rows.sort(key=lambda row: row["cells"][i]["text"], reverse=(j<0))
        return dict(
            page=int(page),
