...
class Saml2Plugin:
    ...
    def __init__(self, config_file, maps=None, logout_url=None, change_password_url=None, entityid=None):
        if maps is None:
            maps = {
    ...
    def get_login_url(self):
        ...
        bindings = [BINDING_HTTP_REDIRECT, BINDING_HTTP_POST]
        binding, destination = client.pick_binding("single_sign_on_service", bindings, "idpsso", entity_id=entityid)
        req_id, req = client.create_authn_request(destination, binding=BINDING_HTTP_POST)
        relay_state = web2py_uuid().replace('-','')
        http_args = client.apply_binding(binding, str(req), destination, relay_state=relay_state)
