...
def stream_file_or_304_or_206(
    ...
    if status == 200:
        ...
        if environ.get('HTTP_IF_MODIFIED_SINCE') == mtime:
            raise HTTP(304, headers={'Content-Type': headers['Content-Type']})

        elif http_range:
            start_items = regex_start_range.findall(http_range)
            if not start_items:
                start_items = [0]
            stop_items = regex_stop_range.findall(http_range)
    ...
    if status != 206:
        enc = environ.get('HTTP_ACCEPT_ENCODING')
