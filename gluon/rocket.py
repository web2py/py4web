...
__all__ = ['VERSION', 'SERVER_SOFTWARE', 'HTTP_SERVER_SOFTWARE', 'BUF_SIZE',
           ...
           'Rocket', 'CherryPyWSGIServer', 'SERVER_NAME', 'NullHandler','HttpServer']
...
# Setup Logging
log_pool = logging.getLogger('Rocket.Errors.ThreadPool')
log_pool.addHandler(NullHandler())
...
class ThreadPool:
    ...
    def __init__(self,
        ...
        if __debug__:
            log_pool.debug("Initializing ThreadPool.")
    ...
    def start(self):
        self.stop_server = False
        if __debug__:
            log_pool.debug("Starting threads.")
    ...
    def stop(self):
        ...
        if __debug__:
            log_pool.debug("Stopping threads.")
        ...
        if has_futures and self.app_info.get('futures'):
            if __debug__:
                log_pool.debug("Future executor is present.  Python will not "
    ...
    def bring_out_your_dead(self):
        ...
        for t in dead_threads:
            if __debug__:
                log_pool.debug("Removing dead thread: %s." % t.getName())
            try:
    ...
    def grow(self, amount=None):
        ...
        if __debug__:
            log_pool.debug("Growing by %i." % amount)
    ...
    def shrink(self, amount=1):
        if __debug__:
            log_pool.debug("Shrinking by %i." % amount)
    ...
    def dynamic_resize(self):
        if (self.max_threads > self.min_threads or self.max_threads == 0):
            ...
            if __debug__:
                log_pool.debug("Examining ThreadPool. %i threads and %i Q'd conxions"
...
class HttpServer(object):
    ...
    def __init__(
        ...
        global SERVER_NAME, SOCKET_TIMEOUT
        if server_name: SERVER_NAME = server_name
        if socket_timeout: SOCKET_TIMEOUT = socket_timeout
        ...
        if not ssl_certificate or not ssl_private_key:
            ...
            log.warning('Python "ssl" module unavailable. SSL is OFF')
        elif not exists(ssl_certificate):
            log.warning('unable to open SSL certificate. SSL is OFF')
        elif not exists(ssl_private_key):
            log.warning('unable to open SSL private key. SSL is OFF')
        else:
        ...
        self.server = Rocket(interfaces or tuple(sock_list),
                             method='wsgi',
                             app_info=app_info,
                             min_threads=min_threads,
                             max_threads=max_threads,
    ...
    def start(self):
        ...
        try:
            ...
            pass
        with open(self.pid_filename,'w') as myfile:
...
if __name__ == '__main__':
    HttpServer(test_wsgi_app).start()
